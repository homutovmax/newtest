import paramiko, json, time, urllib.request

TOKEN = 'CHANGE_ME'

def api(path, method="GET", data=None):
    headers = {"Authorization": f"Bearer {TOKEN}", "Content-Type": "application/json"}
    body = json.dumps(data).encode() if data else None
    req = urllib.request.Request(f"http://192.168.1.92:8123/api/{path}", data=body, headers=headers, method=method)
    try:
        return json.load(urllib.request.urlopen(req, timeout=30))
    except urllib.error.HTTPError as e:
        return {"error": f"{e.code}: {e.read().decode()[:300]}"}

# Тест 1: text
print("1. text")
api("services/media_player/play_media", method="POST", data={
    "entity_id": "media_player.yandex_station_m10ns3600380kb",
    "media_content_id": "Тест. Проверка связи.",
    "media_content_type": "text"
})
time.sleep(10)

# Тест 2: command
print("2. command")
api("services/media_player/play_media", method="POST", data={
    "entity_id": "media_player.yandex_station_m10ns3600380kb",
    "media_content_id": "Скажи: Привет, это тест",
    "media_content_type": "command"
})
time.sleep(10)

# Тест 3: dialog
print("3. dialog")
api("services/media_player/play_media", method="POST", data={
    "entity_id": "media_player.yandex_station_m10ns3600380kb",
    "media_content_id": "Внимание! Повышен уровень углекислого газа",
    "media_content_type": "dialog"
})

print("Готово. Какой вариант сработал?")
